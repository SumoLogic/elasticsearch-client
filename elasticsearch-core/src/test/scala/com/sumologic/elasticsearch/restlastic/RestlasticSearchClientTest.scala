...
class RestlasticSearchClientTest extends WordSpec with Matchers with ScalaFutures with BeforeAndAfterAll
  ...
  "RestlasticSearchClient" should {
    ...
    "Be able to setup document mapping" in {
      val basicFieldMapping = BasicFieldMapping(KeywordType)
      val metadataMapping = Mapping(tpe, IndexMapping(
        Map("name" -> basicFieldMapping, "f1" -> basicFieldMapping, "suggest" -> CompletionMapping(Map("f" -> CompletionContext(CategoryContextType, pathOpt = Some("name"))), Some(analyzerName))), Some(false)))
    ...
    "Be able to setup document mapping with ignoreAbove" in {
      val basicFieldMapping = BasicFieldMapping(KeywordType, ignoreAboveOpt = Some(10000))
      val metadataMapping = Mapping(tpe, IndexMapping(
        Map("name" -> basicFieldMapping, "f1" -> basicFieldMapping, "suggest" -> CompletionMapping(Map("f" -> CompletionContext(CategoryContextType, pathOpt = Some("name"))), Some(analyzerName)))))
    ...
    "Be able to setup document mapping with field index options" in {
      val basicFieldDocsMapping = BasicFieldMapping(
        TextType, analyzerOpt = Some(analyzerName), indexOptionsOpt = Some(DocsIndexOption))
      val basicFieldFreqsMapping = BasicFieldMapping(
        TextType, analyzerOpt = Some(analyzerName), indexOptionsOpt = Some(FreqsIndexOption))
      val basicFieldOffsetsMapping = BasicFieldMapping(
        TextType, analyzerOpt = Some(analyzerName), indexOptionsOpt = Some(OffsetsIndexOption))

      val metadataMapping = Mapping(tpe, IndexMapping(
        Map("f0" -> basicFieldDocsMapping, "f1" -> basicFieldFreqsMapping,
          "f2" -> basicFieldOffsetsMapping, "text" -> basicFieldOffsetsMapping,
          "suggest" -> CompletionMapping(Map("f" -> CompletionContext(CategoryContextType, pathOpt = Some("text"))),
            analyzerOpt = Some(analyzerName)))))
...
//
//      val basicFieldMapping = BasicFieldMapping(TextType, analyzerOpt = Some(analyzerName), searchAnalyzerOpt = Some(analyzerName))
//      val metadataMapping = Mapping(tpe, IndexMapping(
//        Map("name" -> basicFieldMapping, "f1" -> basicFieldMapping, "suggest" -> CompletionMapping(Map("f" -> CompletionContext(CategoryContextType, pathOpt = Some("name"))), analyzerOpt = Some(analyzerName))), Some(false)))
//
//      val mappingFut = restClient.putMapping(index, tpe, metadataMapping)
//      whenReady(mappingFut) { _ => refresh() }
//
...
//      val basicFieldFreqsMapping = BasicFieldMapping(TextType, analyzerOpt = Some(analyzerName),indexOptionsOpt = Some(FreqsIndexOption))
//      val basicFieldOffsetsMapping = BasicFieldMapping(TextType, analyzerOpt = Some(analyzerName), indexOptionsOpt = Some(OffsetsIndexOption))
//
//      val metadataMapping = Mapping(tpe, IndexMapping(
